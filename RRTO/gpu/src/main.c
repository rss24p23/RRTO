...
#define RRTO_PROFILE 1

CUDBGAPI cudbgAPI = NULL;

bool RRTO_print_lane_states(CUDBGAPI cudbgAPI, RRTODeviceProp *dev_prop)
{
    uint64_t warp_mask;
...
bool RRTO_all_warps_broken(CUDBGAPI cudbgAPI, RRTODeviceProp *dev_prop)
{
    uint64_t warp_mask;
    uint64_t warp_mask_broken;
    CUDBGResult res;
...
int RRTO_analyze(int argc, char *argv[])
{
    if (argc != 3) {
    ...
    if (!RRTO_elf_analyze(argv[2])) {
        LOG(LOG_ERROR, "RRTO analyze unsuccessful");
        return -1;
    }
    return 0;
}

int RRTO_restore(int argc, char *argv[])
{
    CUDBGResult res;
    CUDBGAPI cudbgAPI;
    RRTOWarpInfo warp_info = { 0 };
    RRTO_callstack callstack;
    const char *patched_binary = "/tmp/RRTO-ckp/patched_binary";
    const char *ckp_dir = "/tmp/RRTO-ckp";
     ...
     * PC (JMX/BRX).
     */
    if (!RRTO_elf_patch_all(argv[2], patched_binary, &jmptbl, &jmptbl_len)) {
    ...
    for (first_warp = 0; first_warp != 32; first_warp++) {
        ...
        if (RRTO_cr_read_pc(&warp_info, RRTO_CR_NOLANE, ckp_dir,
                               &callstack)) {
            // fprintf(stderr, "RRTO-cr: error while reading pc memory\n");
            break;
        }
    }
    ...
    execute_command("set exec-wrapper env 'LOG=DEBUG' 'LD_PRELOAD=/opt/RRTO/bin/RRTO-server.so' 'RRTO_RESTORE=1'", !batch_flag);
    ...
    uint32_t numDev = 0;
    if (!RRTO_device_get_num(cudbgAPI, &numDev)) {
    ...
    if (!RRTO_device_get_prop(cudbgAPI, 0, &dev_prop)) {
        printf("error getting device properties\n");
        goto detach;
    }
    printf("RRTO: identified device:\n");
    RRTO_device_print_prop(&dev_prop);

    // Wait until all warps have reached the breakpoint at the
    // beginning of the kernel we want to restore
    while (!RRTO_all_warps_broken(cudbgAPI, &dev_prop)) {
        printf("waiting for warps to break...\n");
        usleep(500);
    }
    printf("RRTO: all warps are now stopped!\n");
#ifdef RRTO_PROFILE
    ...
    // printf("RRTO: current lane states:\n");
    // RRTO_print_lane_states(cudbgAPI, &dev_prop);

    // RRTO_cr_kernel_name(cudbgAPI, 0,0,0, &kernel_name);
    // printf("RRTO: kernel-name: \"%s\"\n", kernel_name);

    uint32_t lanemask;
    uint64_t sswarps;
    warp_info.dev = 0;
    warp_info.dev_prop = &dev_prop;
    RRTO_jmptable_index *index;
    RRTO_jmptable_index *kernelindex;
    ...
    if (!RRTO_elf_get_jmptable_index(jmptbl, jmptbl_len, kernel_name,
                                        &kernelindex)) {
    ...
    for (int sm = 0; sm != dev_prop.numSMs; sm++) {
        ...
        if (!RRTO_cr_sm_broken(cudbgAPI, warp_info.dev, sm)) {
            printf("waiting for sm to break...\n");
            while (!RRTO_cr_sm_broken(cudbgAPI, warp_info.dev, sm)) {
                usleep(500);
            }
        }
    }
    ...
    for (int sm = 0; sm != dev_prop.numSMs; sm++) {
        ...
        for (uint64_t warp = 0; warp != dev_prop.numWarps; warp++) {
            if (warp_mask & (1LU << warp)) {
                ...
                if (!RRTO_cr_read_pc(&warp_info, RRTO_CR_NOLANE, ckp_dir,
                                        &callstack)) {
                    printf("RRTO-cr: did not find callstack. exiting "
                           "warp...\n");
                    for (uint32_t lane = 0;
                ...
                for (int c_level = callstack.callstack_size - 1;
                    ...
                    if (index->ssy_num > 0) {
                        ...
                        if (!RRTO_cr_rst_ssy(cudbgAPI, &warp_info,
                    ...
                    if (c_level > 0) {
                        ...
                        if (!RRTO_elf_get_jmptable_index(
                                 jmptbl, jmptbl_len,
    ...
    RRTO_elf_info elf_info;
    RRTO_elf_get_info(kernel_name, &elf_info);
    printf("RRTO: stack-size: %u, param-addr: %u, param-size: %u\n",
           elf_info.stack_size, elf_info.param_addr, elf_info.param_size);

    if (!RRTO_cr_rst_globals(cudbgAPI, ckp_dir)) {
        printf("RRTO: global variable memory restores unsuccessful\n");
    } else {
        printf("RRTO: restored global variables\n");
    }

    if (!RRTO_cr_rst_params(cudbgAPI, ckp_dir, &elf_info, 0, 0, 0)) {
        printf("RRTO: parameter restore unsuccessful\n");
    } else {
        printf("RRTO: restored parameters\n");
    }
    ...
    for (int sm = 0; sm != dev_prop.numSMs; sm++) {
        ...
        for (uint64_t warp = 0; warp != dev_prop.numWarps; warp++) {
            if (warp_mask & (1LU << warp)) {
                ...
                if (!RRTO_cr_rst_shared(cudbgAPI, ckp_dir, &elf_info, 0, sm,
                                           warp)) {
                    printf("RRTO: shared memory restore unsuccessful (size: "
                           "%ld)\n",
                           elf_info.shared_size);
                    continue;
                } else {
                    printf("RRTO: restored shared memory\n");
                }
                ...
                for (uint32_t lane = 0; lane != dev_prop.numLanes; lane++) {
                    if (lanemask & (1LU << lane)) {
                        ...
                        /*if (!(active_lanes & (1<<lane))) {
                            printf("RRTO: lane %u not active (PC: %lx)\n",
                        lane, pc_rb);
                        ...
                        pc_rb !=
                        callstack.pc[0].relative)) {
                            printf("RRTO: lane %u: readback PC (%lx) is not
                        correct
                        (%lx)\n", lane, pc_rb, callstack.pc[0].relative);
                        }*/

                        RRTO_cr_rst_lane(cudbgAPI, &warp_info, lane,
                                            ckp_dir);
                    }
                }
                // printf("RRTO: verified restored PCs\n");
                printf("RRTO: restored warp D%uS%uW%lu\n", 0, sm, warp);
            }
        }
    }
#ifdef RRTO_PROFILE
...
detach:
    RRTO_elf_free_jumptable(&jmptbl, jmptbl_len);
    //RRTO_cr_free_callstack(&callstack);
...
int RRTO_checkpoint(int argc, char *argv[])
{
    char *ckp_dir = "/tmp/RRTO-ckp";
    uint32_t numDev = 0;
    ...
    uint64_t relative_ssy;
    RRTO_callstack callstack;
    RRTO_function_info *function_info = NULL;
   ...
   // attach_command(argv[2], !batch_flag);
    ...
    for (first_warp = 0; first_warp != dev_prop.numWarps; first_warp++) {
        if (*warp_mask & (1LU << first_warp)) {
            if (RRTO_cr_kernel_name(cudbgAPI, 0, 0, first_warp,
                                       &kernel_name)) {
                break;
            }
        }
    ...
    if (kernel_name == NULL) {
        fprintf(stderr, "RRTO-checkpoint: error getting kernel name!\n");
        goto detach;
    } else {
        printf("checkpointing kernel with name: \"%s\"\n", kernel_name);
    }

    if (!RRTO_elf_build_fun_info(&function_info, &fi_num)) {
        ...
        for (uint8_t warp = 0; warp != dev_prop.numWarps; warp++) {
            if (warp_mask[sm] & (1LU << warp)) {
...
#endif
                if (!RRTO_cr_kernel_name(cudbgAPI, 0, sm, warp, &warp_kn)) {
                    fprintf(stderr, "RRTO-checkpoint: could not get kernel "
                                    "name for D%uS%uW%u\n",
                            0, sm, warp);
                    goto detach;
                }
                if (strcmp(warp_kn, kernel_name) != 0) {
                    ...
                    fprintf(stderr,
                            "RRTO-checkpoint: found kernel \"%s\" while "
                ...
                if (!RRTO_cr_callstack(cudbgAPI, &warp_info,
                                          RRTO_CR_NOLANE, &callstack)) {
...
#endif
                if (!RRTO_cr_make_checkpointable(cudbgAPI, &warp_info,
                                                    function_info, fi_num,
                                                    &callstack)) {
                    fprintf(stderr, "RRTO-checkpoint: could not make "
                                    "checkpointable.\n");
                    goto detach;
                }
#ifdef RRTO_PROFILE
                ...
                if (!RRTO_cr_ckp_pc(cudbgAPI, &warp_info, RRTO_CR_NOLANE,
                                       ckp_dir, &callstack)) {
                    fprintf(stderr, "RRTO-checkpoint: ckp_pc failed\n");
                    goto detach;
                }
                RRTO_cr_free_callstack(&callstack);
#ifdef RRTO_PROFILE
                ...
                for (uint8_t lane = 0; lane != dev_prop.numLanes; lane++) {
                    if (callstack.valid_lanes & (1LU << lane)) {
                        RRTO_cr_ckp_lane(cudbgAPI, &warp_info, lane,
                                            ckp_dir);
                    }
                }
#ifdef RRTO_PROFILE
                ...
                if (!RRTO_cr_ckp_shared(cudbgAPI, ckp_dir, &elf_info, 0, sm,
                                           warp)) {
                    printf("RRTO_cr_ckp_shared unsuccessful\n");
                }
#ifdef RRTO_PROFILE
    ...
    gettimeofday(&e, NULL);
#endif
    RRTO_focus_host(!batch_flag);
    execute_command("call (int)RRTO_server_checkpoint(0)", !batch_flag);
    //RRTO_focus_kernel(!batch_flag);


    if (!RRTO_cr_ckp_params(cudbgAPI, ckp_dir, &elf_info, 0, 0,
                               first_warp)) {
        printf("RRTO_cr_ckp_params unsuccessful\n");
    }

    if (!RRTO_cr_ckp_globals(cudbgAPI, ckp_dir)) {
        printf("RRTO_cr_ckp_globals unsuccessful\n");
    }
...
int RRTO_start(int argc, char *argv[])
{
    ...
    //
    execute_command("set exec-wrapper env 'LD_PRELOAD=/opt/RRTO/bin/libtirpc.so.3:/opt/RRTO/bin/RRTO-server.so' 'LOG=DEBUG'", !batch_flag);
...
int main(int argc, char *argv[])
{
    ...
    if (strcmp(argv[1], "start") == 0)
        return RRTO_start(argc, argv);
    if (strcmp(argv[1], "checkpoint") == 0)
        return RRTO_checkpoint(argc, argv);
    if (strcmp(argv[1], "restore") == 0 || strcmp(argv[1], "restart") == 0)
        return RRTO_restore(argc, argv);
    if (strcmp(argv[1], "analyze") == 0)
        return RRTO_analyze(argc, argv);
