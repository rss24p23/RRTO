...
bool RRTO_device_get_num(CUDBGAPI cudbgAPI, uint32_t *dev_num)
{
    ...
    if (res != CUDBG_SUCCESS) {
        fprintf(stderr, "RRTO_device_get_num (%d):%s\n", __LINE__,
                             ...
                             RRTODeviceProp *prop)
{
    ...
    memset(prop, 0, sizeof(struct _RRTODeviceProp));
...
error_name:
    free(prop);
error_prop:
    fprintf(stderr, "RRTO_device_get_prop error: \"%s\"\n",
            cudbgGetErrorString(res));
    return false;
}
...
{
    if (prop == NULL) {
        fprintf(stderr, "RRTO_device_print_prop: prop is NULL\n");
        return;
    }
