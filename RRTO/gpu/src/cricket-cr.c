...
static bool RRTO_cr_gen_suffix(char **suffix, RRTOWarpInfo *wi,
                                  uint32_t lane)
{
    size_t ret;
    if (lane == RRTO_CR_NOLANE) {
                         ...
                         const char *ckp_dir)
{
    ...
    if (!RRTO_cr_gen_suffix(&suffix, wi, lane)) {
        return false;
    }
    ...
    gettimeofday(&b, NULL);
#endif
    register_size = RRTO_register_size(wi->dev_prop);
    ...
    if (!RRTO_file_read_mem(ckp_dir, RRTO_DT_REGISTERS, suffix, reg_mem,
                               register_size)) {
    ...
    stack_loc = ((uint32_t *)reg_mem)[RRTO_stack_get_sp_regnum()];

#ifdef RRTO_PROFILE
    gettimeofday(&c, NULL);
#endif

    if (!RRTO_register_rst(cudbgAPI, wi->dev, wi->sm, wi->warp, lane,
                              reg_mem, wi->dev_prop)) {
        LOGE(LOG_ERROR, "RRTO-cr: error setting register data");
        goto cleanup;
    }
#ifdef RRTO_PROFILE
    gettimeofday(&d, NULL);
    ...
    if (!RRTO_file_read_mem(ckp_dir, RRTO_DT_STACK, suffix, stack_mem,
                               wi->stack_size)) {
    ...
    if (!RRTO_stack_set_mem(cudbgAPI, wi->dev, wi->sm, wi->warp, lane,
                               stack_mem, stack_loc, wi->stack_size)) {
        LOGE(LOG_ERROR, "RRTO-cr: error while retrieving stack memory");
        goto cleanup;
    }
...
#define RRTO_INSTR_SSY_PREFIX 0xe2900000
                        ...
                        const char *ckp_dir)
{
    ...
    for (offset = 0L; offset <= pc; offset += 0x8) {
        ...
        if (((cur_instr >> 32) & 0xfff00000L) == RRTO_INSTR_SSY_PREFIX) {
                        ...
                        RRTO_callstack *callstack)
{
    ...
    if (!RRTO_file_read_mem_size(ckp_dir, RRTO_DT_PC, suffix, &packed, 0,
                                    &packed_size)) {
    ...
    offset += callstack_size * sizeof(RRTO_pc_data);
    if (packed_size < offset) {
    ...
    pc_data = (RRTO_pc_data *)(packed + 3 * sizeof(uint32_t));
    ...
    for (uint32_t lane = 0; lane != wi->dev_prop->numLanes; lane++) {
        if (lanemask & (1LU << lane)) {
            res = cudbgAPI->writeRegister(
                wi->dev, wi->sm, wi->warp, lane, RRTO_JMX_ADDR_REG,
                        ...
                        uint64_t *cur_address)
{
    ...
    if (!RRTO_elf_pc_info(callstack->function_names[c_level],
                             callstack->pc[c_level].relative, &relative_ssy,
                             NULL)) {
        LOGE(LOG_ERROR, "RRTO-restore: getting ssy point failed");
        goto error;
    }
    ...
    if (relative_ssy >= cmp_address) {
        if (!RRTO_elf_get_jmptable_addr(ssy, ssy_num, relative_ssy,
                                           &jmptbl_address)) {
            LOGE(LOG_ERROR, "error getting jmptable adress");
            goto error;
        }
                       ...
                       uint32_t lane_param, RRTO_callstack *callstack)
{
    ...
    if (callstack->callstack_size == 1) {
        ...
        if (callstack->active_lanes != callstack->valid_lanes) {
            LOGE(LOG_ERROR, "RRTO-cr: divergent threads in call levels > 1 "
                            "are not allowed!");
            return false;
        }
        ...
        LOGE(LOG_ERROR, "RRTO-cr: callstacks greater than 2 cannot be "
                        "restored");
        return false;
    }

    if (address > (1ULL << 32)) {
        LOGE(LOG_ERROR, "RRTO-cr: pc value is too large to be restored!");
        goto cleanup;
    }
    ...
    do {
        res = cudbgAPI->writeRegister(wi->dev, wi->sm, wi->warp, lane,
                                    RRTO_JMX_ADDR_REG, ((uint32_t)address));
    ...
    /*res = cudbgAPI->writeRegister(wi->dev, wi->sm, wi->warp, lane,
    RRTO_JMX_ADDR_REG+1, (uint32_t)0x0);
    if (res != CUDBG_SUCCESS) {
                          ...
                          uint32_t lane_param, RRTO_callstack *callstack)
{
    ...
    if ((pc_data = (RRTO_pc_data*)malloc(callstack_size * sizeof(RRTO_pc_data))) == NULL) {
        LOGE(LOG_ERROR, "malloc failed");
        goto cleanup;
    }
    ...
    if (!RRTO_cr_function_name(pc_data[0].virt, &function_names[0])) {
        LOGE(LOG_ERROR, "RRTO-cr: error getting function name");
        goto cleanup;
    }
    ...
    for (i = 1; i < callstack_size; ++i) {
        ...
        if (!RRTO_cr_function_name(pc_data[i].virt, &function_names[i])) {
                       ...
                       RRTO_callstack *callstack)
{
    ...
    packed_size = 3 * sizeof(uint32_t) +
                  callstack->callstack_size * sizeof(RRTO_pc_data) +
                  str_size;
    ...
    memcpy(packed + offset, callstack->pc,
           callstack->callstack_size * sizeof(RRTO_pc_data));
    offset += callstack->callstack_size * sizeof(RRTO_pc_data);
    ...
    if (!RRTO_cr_gen_suffix(&suffix, wi, lane_param)) {
        goto cleanup;
    }

    if (!RRTO_file_store_mem(ckp_dir, RRTO_DT_PC, suffix, packed,
                                packed_size)) {
        fprintf(stderr, "RRTO-cr: error writing pc\n");
        goto cleanup;
    }
                                    ...
                                    RRTO_callstack *callstack)
{
    ...
    RRTO_function_info *the_fi = NULL;
    bool ret = false;
    size_t i;
    bool joined = false;
    bool stepup = false;
    ...
    for (i = callstack->callstack_size - 1; i + 1 > 0; --i) {
        if (!RRTO_elf_get_fun_info(fi, fi_num, callstack->function_names[i],
                                      &the_fi)) {
        ...
        if (!the_fi->has_room) {
            ...
            if (i > 1) {
                ...
                if (!RRTO_cr_sm_broken(cudbgAPI, wi->dev, wi->sm)) {
                    LOGE(LOG_INFO, "waiting for sm to break...");
                    while (!RRTO_cr_sm_broken(cudbgAPI, wi->dev, wi->sm)) {
                        usleep(500);
                    }
                }
                stepup = true;
            ...
            if (!RRTO_elf_pc_info(callstack->function_names[i],
            ...
            if (joined || stepup) {
                ...
                if (!RRTO_cr_callstack(cudbgAPI, wi, RRTO_CR_NOLANE,
                                          callstack)) {
                         ...
                         const char *ckp_dir)
{
    ...
    if (!RRTO_stack_get_mem(cudbgAPI, wi->dev, wi->sm, wi->warp, lane, mem,
                               wi->stack_size)) {
        fprintf(stderr, "RRTO-cr: error while retrieving stack memory\n");
        goto cleanup;
    }
    ...
    if (!RRTO_file_store_mem(ckp_dir, RRTO_DT_STACK, suffix, mem,
                                wi->stack_size)) {
    ...
    if (!RRTO_register_ckp(cudbgAPI, wi->dev, wi->sm, wi->warp, lane, mem,
                              wi->dev_prop)) {
    ...
    if (!RRTO_file_store_mem(ckp_dir, RRTO_DT_REGISTERS, suffix, mem,
                                register_size)) {
                           ...
                           uint32_t sm, uint32_t warp)
{
    ...
    if (!RRTO_file_read_mem(ckp_dir, RRTO_DT_PARAM, NULL, param_mem,
                               elf_info->param_size)) {
    ...
    for (int i = 0; i != elf_info->param_num; ++i) {
        ...
        if (!RRTO_file_exists(ckp_dir, RRTO_DT_HEAP, heap_suffix)) {
            LOGE(LOG_WARNING, "no checkpoint file for parameter %u", i);
            continue;
        }
        param_data = NULL;
        if (!RRTO_file_read_mem_size(ckp_dir, RRTO_DT_HEAP, heap_suffix,
                                        &param_data, 0, &heapsize)) {
            LOGE(LOG_ERROR, "RRTO error while reading heap param data");
            goto cleanup;
        }
        IFLOG(LOG_DBG(3)) {
                           ...
                           uint32_t sm, uint32_t warp)
{
    ...
    if (!RRTO_file_store_mem(ckp_dir, RRTO_DT_PARAM, NULL, param_mem,
                                elf_info->param_size)) {
    ...
    for (int i = 0; i != elf_info->param_num; ++i) {
        ...
        if (!RRTO_heap_memreg_size(
                 *(void **)(param_mem + elf_info->params[i].offset),
                 &heapsize)) {
            LOGE(LOG_DEBUG, "RRTO-heap: param %u is a 64 bit parameter but does not "
        ...
        sprintf(heap_suffix, "-P%u", elf_info->params[i].index);
        if (!RRTO_file_store_mem(ckp_dir, RRTO_DT_HEAP, heap_suffix,
                                    param_data, heapsize)) {
            LOGE(LOG_ERROR, "RRTO error while writing param heap");
        }
    }
    ret = true;
cleanup:
                           ...
                           uint32_t sm, uint32_t warp)
{
    ...
    if (!RRTO_file_store_mem(ckp_dir, RRTO_DT_SHARED, warp_suffix,
                           ...
                           uint32_t sm, uint32_t warp)
{
    ...
    if (!RRTO_file_read_mem(ckp_dir, RRTO_DT_SHARED, warp_suffix,
                               shared_mem, elf_info->shared_size)) {
...
bool RRTO_cr_rst_globals(CUDBGAPI cudbgAPI, const char *ckp_dir)
{
     ...
     * possible?)
     */
    if (!RRTO_elf_get_global_vars_info(&globals, &globals_num)) {
        LOGE(LOG_ERROR, "RRTO-cr: error getting global variable info");
        return false;
    }
    ...
    if (!RRTO_file_read_mem(ckp_dir, RRTO_DT_GLOBALS, NULL, globals_mem,
                               globals_mem_size)) {
    ...
    for (i = 0; i != globals_num; ++i) {
        ...
        if (!RRTO_file_read_mem_size(ckp_dir, RRTO_DT_HEAP, heap_suffix,
            ...
            LOGE(LOG_ERROR, "RRTO error while writing globals heap");
            free(heap_suffix);
            heap_suffix = NULL;
            goto cleanup;
        }
...
bool RRTO_cr_ckp_globals(CUDBGAPI cudbgAPI, const char *ckp_dir)
{
    ...
    if (!RRTO_file_store_mem(ckp_dir, RRTO_DT_GLOBALS, NULL, globals_mem,
                                globals_mem_size)) {
    ...
    for (i = 0; i != globals_num; ++i, offset += globals[i].size) {
        ...
        if (!RRTO_heap_memreg_size(*(void **)(globals_mem + offset),
                                      &heapsize)) {
