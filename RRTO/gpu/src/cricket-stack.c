                          ...
                          uint32_t warp, uint32_t lane, uint32_t *sp)
{
    CUDBGResult res;
    uint32_t sp_val;
    uint32_t sp_regnum = RRTO_stack_get_sp_regnum();
    res = cudbgAPI->readRegister(dev, sm, warp, lane, sp_regnum, &sp_val);
    if (res != CUDBG_SUCCESS) {
        fprintf(stderr, "RRTO-stack (%d) ERROR: %s\n", __LINE__,
                           ...
                           uint32_t stack_size)
{
    uint32_t sp;
    CUDBGResult res;
    if (!RRTO_stack_get_sp(cudbgAPI, dev, sm, warp, lane, &sp)) {
        return false;
    }
