...
cudaError_t cudaGetDevice(int* device)
{
    ...
    if (recorded_count == -1)
    {
        ...
        recorded_count = result.int_result_u.data
        return result.err;
    }
    else
    {
        *device = recorded_count;
        return 0;
    }
}
...
cudaError_t cudaGetLastError(void)
{
    ...
    int result;
    enum clnt_stat retval_1;
    if (recorded_lasterror==-1)
    {
        retval_1 = cuda_get_last_error_1(&result, clnt);
        if (retval_1 != RPC_SUCCESS) {
            clnt_perror (clnt, "call failed");
        }
        recorded_lasterror = result;
        return result;
    }
    else
    {
        return recorded_lasterror;
    }
...
cudaError_t cudaLaunchKernel(const void* func, dim3 gridDim, dim3 blockDim, void** args, size_t sharedMem, cudaStream_t stream)
{
    ...
    api_call_cnt++;
#endif //WITH_API_CNT
    if((output_times ==1)||(operator_count == 0)){
    int result;
    enum clnt_stat retval_1;
    size_t i;
    char *buf;
    ...
    free(rpc_args.mem_data_val);
    recorded_operator_result = result
    return result;
    }
    else{
        return recorded_operator_result;
    }
    operator_count ++;
}
...
cudaError_t cudaHostAlloc(void** pHost, size_t size, unsigned int flags)
{
    ...
    if (shm_enabled && connection_is_local == 1) { //Use local shared memory
        ...
        if (asprintf(&shm_name, "/RRTOhostalloc-%zu", ret.sz_result_u.data) == -1) {
...
cudaError_t cudaMemcpy(void* dst, const void* src, size_t count, enum cudaMemcpyKind kind)
{
    ...
    if (kind == cudaMemcpyHostToDevice) {
        ...
        if(input_times != output_times)output_times ++;
