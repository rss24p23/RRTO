...
static void rpc_connect(void)
{
    ...
    prog = 99;
    vers = 1;
    const char *env_vers = getenv("RRTO_RPCID");
    if (env_vers != NULL) {
        if (sscanf(env_vers, "%lu", &vers) != 1) {
            LOGE(LOG_ERROR, "error parsing RRTO_RPCID");
            exit(1);
        }
    }
    ...
    if (filename != NULL) {
        for (size_t i=0; i != replace_libs_sz; ++i) {
            if (strcmp(filename, replace_libs[i]) == 0) {
                LOG(LOG_DEBUG, "replacing dlopen call to %s with RRTO-client.so", filename);
                dl_handle = dlopen_orig("RRTO-client.so", flag);
                if (clnt == NULL) {
