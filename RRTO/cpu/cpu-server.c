...
int RRTO_server_checkpoint(int dump_memory)
{
    int ret;
...
{
    int ret;
    if ((ret = RRTO_server_checkpoint(1)) != 0) {
        LOGE(LOG_ERROR, "failed to create checkpoint");
    }
    return ret == 0;
}
...
*/
void RRTO_so_register(void* dlhandle, char *path)
{
    // struct link_map *map;
...
bool_t rpc_dlopen_1_svc(char *path, int *result, struct svc_req *rqstp)
{
    ...
    if ((dlhandle = dlopen(path, RTLD_LAZY)) == NULL) {
       ...
       //RRTO_so_register(dlhandle, path);
...
void RRTO_main(size_t prog_num, size_t vers_num)
{
    int ret = 1;
    ...
    if (getenv("RRTO_DISABLE_RPC")) {
        LOG(LOG_INFO, "RPC server was disable by setting RRTO_DISABLE_RPC");
        return;
    }
    if (getenv("RRTO_RESTORE")) {
        LOG(LOG_INFO, "restoring previous state was enabled by setting RRTO_RESTORE");
            restore = 1;
    }
