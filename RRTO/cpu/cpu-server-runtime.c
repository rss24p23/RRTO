...
bool_t replay_each_operator(tr func, rpc_dim3 gridDim, rpc_dim3 blockDim,
...
ptr recorded_func[1000];
rpc_dim3 recorded_gridDim[1000];
rpc_dim3 recorded_blockDim[1000];
mem_data recorded_args[1000];
size_t recorded_sharedMem[1000];
ptr recorded_stream[1000];
                                ...
                                int *result, struct svc_req *rqstp)
{
    if (input_times == 1)
    {
        recorded_func[operator_count] = func;
        recorded_gridDim[operator_count] = gridDim;
        recorded_blockDim[operator_count] = blockDim;
        recorded_args[operator_count] = args;
        recorded_sharedMem[operator_count] = sharedMem;
        recorded_stream[operator_count] = stream;
        replay_each_operator(func, gridDim,blockDim,args,sharedMem,stream,result,rqstp);
    }
    else{
        for (int i =0;i<operator_count;i++)
        {
            ptr func_tmp = recorded_func[i];
            rpc_dim3 gridDim_tmp = recorded_gridDim[i];
            rpc_dim3 blockDim_tmp = recorded_blockDim[i];
            mem_data args_tmp = recorded_args[i];
            size_t sharedMem_tmp = recorded_sharedMem[i];
            ptr stream_tmp = recorded_stream[i];
            replay_each_operator(func_tmp, gridDim_tmp,blockDim_tmp,args_tmp,sharedMem_tmp,stream_tmp,result,rqstp);
        }input_address_old[input_count]
    }
}
...
bool_t cuda_host_alloc_1_svc(size_t size, unsigned int flags, sz_result *result, struct svc_req *rqstp)
{
    ...
    if (socktype == UNIX || (shm_enabled && cpu_utils_is_local_connection(rqstp))) { //Use local shared memory
        if (asprintf(&shm_name, "/RRTOhostalloc-%d", hainfo_cnt) == -1) {
        ...
        LOGE(LOG_ERROR, "cudaHostAlloc is not supported for other transports than UNIX. This error means RRTO_server and RRTO_client are not compiled correctly (different transports)");
        goto out;
    }
...
bool_t cuda_host_alloc_regshm_1_svc(size_t hainfo_idx, ptr client_ptr, int *result, struct svc_req *rqstp)
{
    ...
    *result = cudaErrorMemoryAllocation;
    ...
    if (asprintf(&shm_name, "/RRTOhostalloc-%d", hainfo_idx) == -1) {
        LOGE(LOG_ERROR, "asprintf failed: %s", strerror(errno));
        goto out;
    }
...
uint64_t input_address[100];
int input_count = 0;
bool_t cuda_memcpy_htod_1_svc(uint64_t ptr, mem_data mem, size_t size, int *result, struct svc_req *rqstp)
{
    if(input_times == output_times)input_times ++;
    if(output_times==0){
        input_address[input_count] = ptr;
    }
    else{
        ptr = input_address[input_count];
    }
    input_count ++;
